...
def inference_window_all_chunk_whole(path_list_gz, newpath_flow_forward, newpath_flow_backward, newpath_registered_forward, newpath_registered_backward, model, image_size, es_number):
...
#
        ...
        for cn, chunk in enumerate([chunk1_0, chunk2_0]):
            ...
            backward_flow_pred = out2['cumulated'].detach().to('cpu')
            #del chunk_x
    ...
    for d in range(x.shape[-1]):
        ...
        for cn, chunk in enumerate([chunk1_0, chunk2_0]):
            ...
            for t in range(len(chunk_x) - 1):
                ...
                out1, out2 = model(x_in, inference=False)
...
def inference_iterative_syn(path_list_gz, newpath_flow, newpath_registered, es_number):
    ...
    moved_list_all = []
    img_list_all = []
    registered_list_all = [] #data: (D, T, ...)
    for d in range(x.shape[-1]):
        current_fixed = x[0, :, :, :, :, d]

        indices = torch.arange(1, x.shape[0])
        chunk1 = indices[:es_idx-1]
        chunk2 = indices[es_idx-1:]
        chunk2 = torch.flip(chunk2, dims=[0])
        ...
        chunk_list_flow = []
        chunk_list_seg = []
        chunk_list_registered = []

        for cn, chunk in enumerate([chunk1_0, chunk2_0]):
            chunk_x = x[chunk]
            chunk_seg = gt[chunk]
            chunk_path = np.array(path_list_gz[chunk])

            out_list = []
            out_list_flow = []
            out_list_registered = []
            for t in range(len(chunk_path)):
                moving_path = chunk_path[t]
                if fixed_path == moving_path:
                    continue
                filename = os.path.basename(moving_path)

                current_moving = chunk_x[t, :, :, :, :, d]
                moving_seg = chunk_seg[t, :, :, :, :, d]
            
                #x = torch.stack([current_fixed, current_moving], dim=0)
                #x = NormalizeIntensity()(x)

                moving_image = ants.from_numpy(current_moving[0, 0].numpy())
                fixed_image = ants.from_numpy(current_fixed[0, 0].numpy())
                ...
                assert registered_img.min() >= 0.0
                assert registered_img.max() <= 1.0

                # Warp the other image using the deformation field
                ed_target = torch.nn.functional.one_hot(moving_seg[:, 0].long(), num_classes=4).permute(0, 3, 1, 2).contiguous().float()[0]
                class_list = []
                for c in range(4):
                    current_mask = ants.from_numpy(ed_target[c].numpy())
                    current_moved = ants.apply_transforms(
                        fixed=fixed_image,
                        moving=current_mask,
                        transformlist=registration['fwdtransforms'],
                        interpolator='linear'
                    )
                    class_list.append(current_moved.numpy())
                
                moved = np.stack(class_list, axis=0)[None]
                #moved = np.argmax(moved, axis=1, keepdims=True).astype(int) # B, 1, H, W

                # Load the deformation field image
                deformation_field_image = ants.image_read(registration['fwdtransforms'][0]).numpy()

                #fig, ax = plt.subplots(1, 3)
                ##arr = arr.transpose((1, 0, 2, 3))
                #step_quiver = 4
                #X, Y = np.meshgrid(np.arange(0, deformation_field_image.shape[0], step=step_quiver), np.arange(deformation_field_image.shape[1], step=step_quiver))
                #ax[0].imshow(current_moving[0, 0].cpu(), cmap='gray')
                #ax[1].imshow(moved[0, 0], cmap='gray')
                #ax[2].imshow(torch.argmax(ed_target, dim=0).cpu(), cmap='gray')
                #ax[0].quiver(X, Y, deformation_field_image[::step_quiver, ::step_quiver, 0], deformation_field_image[::step_quiver, ::step_quiver, 1], color='r', angles='xy', scale_units='xy', scale=1)
                #plt.show()

                out_list.append(moved[0])
                out_list_flow.append(deformation_field_image)
                out_list_registered.append(registered_img)
        
            moved = np.stack(out_list, axis=0)
            flow = np.stack(out_list_flow, axis=0)
            registered_sequence = np.stack(out_list_registered, axis=0)

            chunk_list_flow.append(flow)
            chunk_list_seg.append(moved)
            chunk_list_registered.append(registered_sequence)
        
        nan_tensor_1 = chunk_list_flow[0]
        nan_tensor_2 = chunk_list_flow[1]
        
        warp = np.concatenate([nan_tensor_1, np.flip(nan_tensor_2, axis=0)], axis=0)

        assert len(warp) == x.shape[0] - 1

        nan_tensor_1 = chunk_list_seg[0]
        nan_tensor_2 = chunk_list_seg[1]
        
        moved = np.concatenate([nan_tensor_1, np.flip(nan_tensor_2, axis=0)], axis=0)

        nan_tensor_1 = chunk_list_registered[0]
        nan_tensor_2 = chunk_list_registered[1]
        
        registered_sequence = np.concatenate([nan_tensor_1, np.flip(nan_tensor_2, axis=0)], axis=0)

        flow_list_all.append(warp)
        moved_list_all.append(moved)
        img_list_all.append(x[1:, 0, 0, :, :, d])
        registered_list_all.append(registered_sequence)
    ...
    registered = np.stack(registered_list_all, axis=-1) # T-1, H, W, D
    ...
    for t in range(len(moved)):
        ...
        flow_filename = os.path.basename(moving_path)[:-4] + '.npz'
        save_path_flow = os.path.join(newpath_flow, flow_filename)
        np.savez(save_path_flow, flow=flow[t].squeeze(), img=img[t].squeeze(), registered=registered[t])
...
network_1 = build_flow_model_successive(config, image_size=image_size, log_function=None, nb_channels=1, backward=False, segmentation=config['segmentation'])
if config['video_length'] > 2:
    nb_channels = 6 if not config['no_error'] else 4
    network_2 = build_flow_model_successive(config, image_size=image_size, log_function=None, nb_channels=nb_channels, backward=False, segmentation=False)
else:
    network_2 = None
model = ModelWrap(model1=network_1, model2=network_2, do_ds=config['deep_supervision'], motion_from_ed=config['motion_from_ed'], backward=False, segmentation=config['segmentation'], no_error=config['no_error'], use_label=False)
...
for (path_list_gz, path_list_pkl) in tqdm(zip(all_patient_paths, all_patient_paths_pkl), total=len(all_patient_paths)):
    ...
    with torch.no_grad():
        #inference_iterative_syn(path_list_gz, newpath_flow_backward, newpath_registered_backward, es_number)
        if config['video_length'] > 2:
            inference_window_all_chunk_whole(path_list_gz, newpath_flow_forward, newpath_flow_backward, newpath_registered_forward, newpath_registered_backward, model, image_size, es_number)
        else:
